...
#   alg2 <- OSSClassif(season~., clean.algae, Cl=c("spring", "summer"), start="Tomek")
#   alg3 <- OSSClassif(season~., clean.algae, start="CNN")
#   alg4 <- OSSClassif(season~., clean.algae, start="Tomek")
#   alg5 <- OSSClassif(season~., clean.algae, dist="p-norm", p=1, use.at="numeric", Cl="winter")
#   summary(alg1$season)
#   summary(alg2$season)
#   summary(alg3$season)
#   summary(alg4$season)
...
# P. Branco, April 2015
# ---------------------------------------------------
OSSClassif <- function(form, data, dist="Euclidean", p=2, use.at="all", Cl="smaller", start="CNN") 
  # INPUTS:
...

{
  ...
  return(d2[[1]])  
    ...
    d1 <- TomekClassif(form, data, dist=dist, p=p, use.at=use.at, Cl=otherCl, rem="both")
    
    d2 <- CNNClassif(form, d1[[1]], dist=dist, p=p, use.at=use.at, Cl=Cl)
    return(d2[[1]])
