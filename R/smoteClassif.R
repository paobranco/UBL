...
## ---------------------------------------------------
smoteClassif <- function(form,data, perc.over=2,k=5,perc.under=2,, repl=FALSE)
...

{
  ...
  newExs <- smote.exsClassif(data[minExs,],ncol(data),perc.over,k)
  if (tgt < ncol(data)) {
      newExs <- newExs[,cols]
      data <- data[,cols]
  }
  ...
  selMaj <- sample((1:NROW(data))[-minExs],
                   as.integer(perc.under*nrow(newExs)+length(minExs)),
                   replace=repl)
  ...
  # examples with rare class on the target
{
  ...
  # todo: check if it is this what we really want
