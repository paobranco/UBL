...
## P.Branco, May 2015
## ---------------------------------------------------
gaussNoiseClassif <- function(form, data, C.perc="balance", pert=0.1, repl=FALSE)
  ...
  #      examples. Assuming as center the base example, this parameter defines
  #      radius (based on the standard deviation) where the new example is generated. 
  ...
  if(is.list(C.perc)){
    ...
    if(length(names.ove)){ # perform over-sampling
      for(i in 1:length(names.ove)){
        newExs <- gn.exsClassif(data[which(data[,ncol(data)] == names.ove[i]),], ncol(data), C.perc[[names.ove[i]]], pert)
    ...
    if(length(ove)){ #perform over-sampling 
      for(i in 1:length(ove)){
        newExs <- gn.exsClassif(data[which(data[,ncol(data)] == li[[1]][ove[i]]),], ncol(data), li[[3]][ove[i]]/li[[2]][ove[i]]+1, pert)
        # add original rare examples and synthetic generated examples
...
# P.Branco, May 2015
# ---------------------------------------------------
gn.exsClassif <- function(data, tgt, N, pert)
  # INPUTS:
  # data are the minority class cases
  ...
  # examples with rare class on the target
{
  ...
  if(extra){
    ...
    for (i in id.ex){
      ...
      for (num in 1:(nC-1)){
        newdata[nexs*nL+count, num] <- T[i,num]+rnorm(1, 0, sd(T[,num])*pert)
        if(num %in% nomatr){
          probs <- c()
          for(u in 1:length(unique(T[,num]))){
            probs <- c(probs,length(which(T[,num]==unique(T[,num])[u])))
