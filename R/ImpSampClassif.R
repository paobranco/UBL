...
# both random over and undersampling
# 
# Examples:
...
# P. Branco, July 2015
# ---------------------------------------------------
ImpSampClassif <- function(form, data, C.perc="balance")
...

{
  ...
  if(is.list(C.perc)){
    ...
    # include examples from classes unchanged
    newdata <- data[which(data[,tgt] %in% li[[1]][same]),]
    
    if(length(und)){ #perform under-sampling
      for(i in 1:length(und)){ 
        Exs <- which(data[,tgt]== li[[1]][und[i]])
        sel <- sample(Exs,
