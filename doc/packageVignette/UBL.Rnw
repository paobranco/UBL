...
install_github("paobranco/UBL",ref="master") # to install the stable release
@
...
<<message=FALSE, eval=FALSE>>=
library(devtools)  # You need to install this package!
install_github("paobranco/UBL",ref="develop") # to install the development release
@
...
install_git("https://github.com/paobranco/UBL",branch="develop")
...
This strategy is available in \UBL through the function \texttt{ImpSampClassif}. The user may specify using parameter \texttt{C.perc} the classes where over-/under-sampling must be aaplied, by indicating the corresponding percentages. If all classes are equally important and a perfectly balanced data set should be obtained, the \texttt{C.perc} parameter must be set to "balance". On the other hand, if the classes frequencies sould be inverted, then this parameter should be "extreme". The following example illustrate the use of this function.
...
nds <- ImpSampClassif(Species~.,data, C.perc=list(setosa=0.5, virginica=6))
# notice that when a certain class is not specified it remains unaltered
table(nds$Species)

# to obtain a balanced data set
IS.bal <- ImpSampClassif(Species~., data) # or use C.perc="balance"
table(IS.bal$Species)

# to obtain a data set with inverted frequencies 
IS.ext <- ImpSampClassif(Species~., data, C.perc="extreme")
...
# visualizing the two classes that have changed (virginica and versicolor)
  ...
  par(mfrow = c(2, 2))
  plot(data[, 1], data[, 2], pch =as.integer(data[, 3]), col=as.integer(data[,3]), xlim=c(4,8), ylim=c(2,4.5),
...
DF <- rbind(data.frame(Dat="Original",obs=data$Species), data.frame(Dat="nds", obs=nds$Species), data.frame(Dat="IS.bal", obs=IS.bal$Species), data.frame(Dat="IS.ext", obs=IS.ext$Species))
g1 <- ggplot(DF,aes(x=obs, fill=Dat, colour=Dat))+geom_histogram(binwidth=1, position="dodge", aes(group=Dat), colour="black")
