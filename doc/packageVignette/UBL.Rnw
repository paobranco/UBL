...
Clearly, the model presents a poor performance on least represented class, the \Sexpr{names(which(table(test[,3])==min(table(test[,3]))))} class. However, for the most common class, the \Sexpr{names(which(table(dat[,3])==max(table(dat[,3]))))} the learner always predicts correctly.
...
We can observe that the least represented class, \Sexpr{names(which(table(test[,3])==min(table(test[,3]))))}, now presents an improved result.
...
<<Iris_RU1,fig.cap="The impact of random under-sampling strategy.",out.width="0.8\\textwidth", echo=FALSE>>=
  par(mfrow = c(1, 2))
  plot(dat[, 1], dat[, 2], pch = as.integer(dat[, 3]), col=as.integer(dat[,3]),
       main = "Original Data")
...
res <- c(table(dat$Species),table(RUmy.ir$Species), table(RUB.ir$Species), table(RUE.ir$Species))

m <- matrix(res, nrow=4,ncol=3, byrow=TRUE, dimnames=list(nm, names(table(dat$Species))))
...
DF <- rbind(data.frame(Dat="Original",obs=dat$Species), data.frame(Dat="RUmy.ir", obs=RUmy.ir$Species), data.frame(Dat="RUB.ir", obs=RUB.ir$Species), data.frame(Dat="RUE.ir", obs=RUE.ir$Species))
g1 <- ggplot(DF,aes(x=obs, fill=Dat, colour=Dat))+geom_bar(stat="count", position="dodge", aes(group=Dat), colour="black")
...
res <- c(table(dat$Species),table(RO.U1$Species), table(RO.U2$Species), table(RO.B$Species), table(RO.E$Species))
...
DF <- rbind(data.frame(Dat="Original",obs=dat$Species), data.frame(Dat="RO.U1", obs=RO.U1$Species), data.frame(Dat="RO.B", obs=RO.B$Species), data.frame(Dat="RO.E", obs=RO.E$Species))
g1 <- ggplot(DF,aes(x=obs, fill=Dat, colour=Dat))+geom_bar(position="dodge", aes(group=Dat), colour="black")
...
<<ISC, fig.cap="Impact of Importance Sampling strategy in Iris unbalanced data set.", echo=FALSE>>=
# visualizing the two classes that have changed (virginica and versicolor)

  par(mfrow = c(2, 2))
  plot(dat[, 1], dat[, 2], pch =as.integer(dat[, 3]), col=as.integer(dat[,3]), xlim=c(4,8), ylim=c(2,4.5),
       main = "Original Unbalanced Data")
  plot(jitter(nds[, 1]), jitter(nds[, 2]), pch=as.integer(nds[,3]), col=as.integer(nds[,3]),xlim=c(4,8),ylim=c(2,4.5),
       main = "Importance Sampling with user defined parameter")
  plot(jitter(IS.bal[, 1]), jitter(IS.bal[, 2]), pch=as.integer(IS.bal[,3]), col=as.integer(IS.bal[,3]),xlim=c(4,8),ylim=c(2,4.5),
       main = "Importance Sampling for balancing the data set")
  plot(jitter(IS.ext[, 1]), jitter(IS.ext[, 2]), pch=as.integer(IS.ext[,3]), col=as.integer(IS.ext[,3]),xlim=c(4,8),ylim=c(2,4.5),
       main = "Importance Sampling for invert the class frequencies")
...
DF <- rbind(data.frame(Dat="Original",obs=dat$Species), data.frame(Dat="nds", obs=nds$Species), data.frame(Dat="IS.bal", obs=IS.bal$Species), data.frame(Dat="IS.ext", obs=IS.ext$Species))
g1 <- ggplot(DF,aes(x=obs, fill=Dat, colour=Dat))+geom_bar(position="dodge", aes(group=Dat), colour="black")
...
par(mfrow=c(2,2))
plot(dat[,1], dat[,2], col=as.integer(dat[,3]), main="original data")
...
res <- c(table(dat$Species),table(ir2$Species), table(ir3$Species), table(ir4$Species))
...
nm2 <- list(dat, irB, irE,irU1, irU2)

res <- c(table(dat$Species), table(irB$Species), table(irE$Species), table(irU1$Species), table(irU2$Species))
...
DF <- rbind(data.frame(Dat="Original",obs=dat$Species), data.frame(Dat="irB", obs=irB$Species), data.frame(Dat="irE", obs=irE$Species), data.frame(Dat="irU1", obs=irU1$Species), data.frame(Dat="irU2", obs=irU2$Species))
g1 <- ggplot(DF,aes(x=obs, fill=Dat, colour=Dat))+geom_bar( position="dodge", aes(group=Dat), colour="black")
...
res <- c(table(dat$Species), table(mysmote1$Species), table(mysmote2$Species), table(mysmote3$Species), table(smoteB$Species), table(smoteE$Species))

m <- matrix(res, nrow=6,ncol=3, byrow=TRUE, dimnames=list(nm, names(table(dat$Species))))
...
DF <- rbind(data.frame(Dat="Original",obs=dat$Species), data.frame(Dat="mysmote1", obs=mysmote1$Species), data.frame(Dat="mysmote2", obs=mysmote2$Species), data.frame(Dat="mysmote3", obs=mysmote3$Species), data.frame(Dat="smoteB", obs=smoteB$Species), data.frame(Dat="smoteE", obs=smoteE$Species))
g1 <- ggplot(DF,aes(x=obs, fill=Dat, colour=Dat))+geom_bar( position="dodge", aes(group=Dat), colour="black")
