...
This strategy is available in \UBL through the function \texttt{ImpSampClassif}. The user may specify using parameter \texttt{C.perc} the classes where over-/under-sampling must be aaplied, by indicating the corresponding percentages. If all classes are equally important and a perfectly balanced data set should be obtained, the the \texttt{C.perc} parameter must be set to "balance". On the other hand, if the classes frequencies sould be inverted, then this parameter should be "extreme". The following example illustrate the use of this function.
...
\end{verbatim}
\begin{alltt}
\hlstd{nds} \hlkwb{<-} \hlkwd{ImpSampClassif}\hlstd{(Species}\hlopt{~}\hlstd{.,data,} \hlkwc{C.perc}\hlstd{=}\hlkwd{list}\hlstd{(}\hlkwc{setosa}\hlstd{=}\hlnum{0.5}\hlstd{,} \hlkwc{virginica}\hlstd{=}\hlnum{6}\hlstd{))}
...
\hlstd{IS.bal} \hlkwb{<-} \hlkwd{ImpSampClassif}\hlstd{(Species}\hlopt{~}\hlstd{., data)} \hlcom{# or use C.perc="balance"}
...
\hlstd{IS.ext} \hlkwb{<-} \hlkwd{ImpSampClassif}\hlstd{(Species}\hlopt{~}\hlstd{., data,} \hlkwc{C.perc}\hlstd{=}\hlstr{"extreme"}\hlstd{)}
